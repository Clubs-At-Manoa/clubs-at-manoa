...
function getProjectData(name) {
  ...
  const interests = _.pluck(ClubTypes.collection.find({ project: name }).fetch(), 'interest');
  const profiles = _.pluck(ProfilesClubs.collection.find({ project: name }).fetch(), 'profile');
...
const ProjectsPage = () => {
  const { ready } = useTracker(() => {
    ...
    const sub3 = Meteor.subscribe(ClubTypes.userPublicationName);
