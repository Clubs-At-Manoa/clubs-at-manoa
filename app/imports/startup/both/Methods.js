...
import { ClubTypes } from '../../api/clubs/ClubTypes';
...
/** Creates a new project in the Clubs collection, and also updates ProfilesClubs and ClubTypes. */
Meteor.methods({
  'Projects.add'({ name, description, picture, interests, participants, homepage }) {
    ...
    ClubTypes.collection.remove({ project: name });
    if (interests) {
      interests.map((interest) => ClubTypes.collection.insert({ project: name, interest }));
    } else {
      throw new Meteor.Error('At least one interest is required.');
    }
    if (participants) {
      participants.map((participant) => ProfilesClubs.collection.insert({ project: name, profile: participant }));
    }
  },
});
